  ...
  return {
    ...
    , twitter: (function(){
      ...
      function render(tweets, twitter_user) {
        var timeline = document.getElementById('tweets'),
            content = '<li class="nav-header">Latest Tweets</l1>';

        for (var t in tweets) {
          content += '<li>'+'<p>'+'<a href="http://twitter.com/'+twitter_user+'/status/'+tweets[t].id_str+'" style="margin-right: 10px">'+octopress.prettyDate(tweets[t].created_at)+'</a>'+linkifyTweet(tweets[t].text.replace(/\n/g, '<br>'), tweets[t].entities.urls)+'</p>'+'</li>';
        }
    ...
    , github: (function(){
      ...
      function render(target, data){
        var i = 0, repos = '<li class="nav-header">On GitHub</li>';
